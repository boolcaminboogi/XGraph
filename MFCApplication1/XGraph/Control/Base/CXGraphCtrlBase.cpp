...
void CXGraphCtrlBase::OnPaint()
{
	...
	else
	{
		...
		CSize textSize;
		double dPitch = (double)(m_configDataLine.m_dDisplayMax - m_configDataLine.m_dDisplayMin) / (double)m_configGrid.m_GridCount.y;
		...
		for (int i = 0; i <= m_configGrid.m_GridCount.y; i++)
		{
			...
			if (m_configGrid.m_bShowUnit)
			{
				if (i % m_configGrid.m_nDisplayFrequency == 0)
				{
					...
					{
						strDisplayValue.Format(_T("%d"), (int)(m_configDataLine.m_dDisplayMax - dPitch * i));
					}
					else
					{
						strFormat.Format(_T("%%.%df"), m_configDataLine.m_nFormat);
						strDisplayValue.Format(strFormat, m_configDataLine.m_dDisplayMax - dPitch * i);
					}
