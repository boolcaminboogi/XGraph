...
void CXGraphCtrl::SetTitle(CString strTitle)
{
	m_strTitle = strTitle;
}

void CXGraphCtrl::SetDescription_of_the_unit(CString strUnit)
{
	m_strDescription_of_the_unit = strUnit;
}

// Display Data Min, Max 세팅
void CXGraphCtrl::SetDisplayData(double dMin, double dMax)
{
	m_dDisplayMin = dMin;
	m_dDisplayMax = dMax;
}
...
void CXGraphCtrl::OnPaint_DataLine(CDC* pDC)
{
	if (pDC)
	{
		...
		//---------------------------------
		// 텍스트의 가로, 세로 크기 측정
		CSize textSize = pDC->GetTextExtent(m_strTitle);
		...
		// [0] Unit 표시
		//---------------------------------
		textSize = pDC->GetTextExtent(m_strDescription_of_the_unit);
#if 1
		pDC->TextOutW(
			graph_rect.left,
		...
		{
			if (m_dSpecMin < m_dDisplayMax &&
				m_dSpecMax > m_dDisplayMin)
			{
				// 펜
				...
				// Draw the spec line
				if (m_dSpecMin > m_dDisplayMin)
				{
					y = (int)(((double)graph_rect.Height()) * (m_dSpecMin - m_dDisplayMin) / (m_dDisplayMax - m_dDisplayMin));
					...
					{
						strSpec.Format(_T("%d"), (int)m_dSpecMin);
					}
					else
					{
						strFormat.Format(_T("%%.%df"), m_configDataLine.m_nFormat);
						strSpec.Format(strFormat, m_dSpecMin);
					}
					textSize = pDC->GetTextExtent(strSpec);
#if 0
					pDC->TextOutW(
				...
				if (m_dSpecMax < m_dDisplayMax)
				{
					y = (int)((double)graph_rect.Height() * (m_dSpecMax - m_dDisplayMin) / (m_dDisplayMax - m_dDisplayMin));
					y = graph_rect.Height() - y + graph_rect.top;
					pDC->MoveTo(graph_rect.left, y);
					pDC->LineTo(graph_rect.right, y);
					...
					if (m_configDataLine.m_nFormat == 0)
					{
						strSpec.Format(_T("%d"), (int)m_dSpecMax);
					}
					else
					{
						strFormat.Format(_T("%%.%df"), m_configDataLine.m_nFormat);
						strSpec.Format(strFormat, m_dSpecMax);
					}
					textSize = pDC->GetTextExtent(strSpec);
#if 0
					// 오른쪽에 표시
		...
		if (m_data.size() > 0)
		{
			...
			int x_start = x;
			int y = (int)((double)graph_rect.Height() * (m_data[0] - m_dDisplayMin) / (m_dDisplayMax - m_dDisplayMin));
			y = graph_rect.Height() - y + graph_rect.top;
			pDC->MoveTo(x, y);
			...
			for (size_t i = 1; i < m_data.size(); i++)
			{
				...
				y = (int)((double)graph_rect.Height() * (m_data[i] - m_dDisplayMin) / (m_dDisplayMax - m_dDisplayMin));
				y = graph_rect.Height() - y + graph_rect.top;
				pDC->LineTo(x, y);

				// 점 표시 (이전 값을 표시한다.)
				if (m_configDataLine.m_bUseSpecLine && (m_data[i - 1] < m_dSpecMin || m_data[i - 1] > m_dSpecMax))
				{
			...
			if (m_data.size() > 0)
			{
				int idxLast = m_data.size() - 1;
				if (m_configDataLine.m_bUseSpecLine && (m_data[idxLast] < m_dSpecMin || m_data[idxLast] > m_dSpecMax))
				{
					pDC->FillSolidRect(&dot_rect, RGB(255, 0, 0)); // 빨강색 채우기
				}
				else
